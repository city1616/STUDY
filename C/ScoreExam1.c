...
int main() {
   ...
   int stNum;          //학생수(학생번호)
   int i=0;              //학생수 체크, 점수 체크, 실행을 마친 후 프로그램을 종료하기 전에 입력할 정수
   int NumScore[30][4];             //순번, 수시1, 수시2, 수시3 배열 생성   //행 : 학생수 , 열 : 4(순번, 수시1, 수시2, 수시3)
   ...
   if (CheckstNum(stNum) == 1) {            //학생수가 1이상 30이하인지 검사하는 함수 호출
      printf("학생수 오류!!\n");
      printf("아무 숫자나 누르시오!!");
      scanf("%d", &i);                    //종류하기전 정수 입력
      return 0;                           //프로그램 종료
   }


   for (i = 0; i < stNum; i++) {        //학생수 만큼 반복해서 순번, 수시1, 수시2, 수시3을 입력을 받음
...
void printList(int stNum, int NumScore[][4]) {      //학생수와 순번, 수시점수를 받아와서 출력하는 함수
   ...
   int Sum[30];                                 //학생별 수시1, 수시2, 수시3의 총합계를 저장할 배열
   double Average[30];                           //학생별 수시1, 수시2, 수시3의 평균을 저장할 배열
   char Grade[30];                              //학생별 학점을 저장할 배열
   double avg[3];                                   //수시1, 수시2, 수시3의 평균을 저장할 배열
   int Max[4] = { 0 };                               //수시1, 수시2, 수시3, 총점의 최고점을 저장할 배열
   int Min[4] = { 10000, 10000, 10000, 10000 };      //수시1, 수시2, 수시3, 총점의 최저점을 저장할 배열
   double Maxavg = 0, Minavg = 10000;               //수시1, 수시2, 수시3의 평균의 최고점, 최저점을 저장할 실수형 선언
