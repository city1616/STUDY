...
#load overlay image
overlay = cv2.imread('ironman_mask.png', cv2.IMREAD_UNCHANGED)
...
while True :
	...
	face_size = int(max(bottom_right - top_left) * 2.0)

	center_x, center_y = np.mean(shape_2d, axis = 0).astype(np.int)

	result = overlay_transparent(ori, overlay, center_x + 18, center_y - 25, overlay_size = (face_size, face_size))
